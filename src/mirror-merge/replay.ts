...
import { runGit, runGitStdin, runGitText } from '../git/index.ts';
...
export function applyUpstreamCommitToIndex(input: {
  ...
  if (removePaths.length > 0) {
    runGitStdin(
      ...
      ['rm', '--cached', '-r', '-f', '--ignore-unmatch', '--pathspec-from-file=-'],
      `${removePaths.join('\n')}\n`
    );
  }
...
function formatGitReplayDateEnv(unixSeconds: number): string {
  return `@${unixSeconds}`;
}
